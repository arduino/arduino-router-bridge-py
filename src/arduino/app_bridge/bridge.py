...
# Standard library logger; embedding applications may configure or replace it (see set_logger)
logger = logging.getLogger(__name__)


_address_resolver = None  # Optional callable mapping a requested router address to the effective one
...
def set_address_resolver(resolver: callable):
    """Sets a hook that maps the requested router address to the effective one.

    The resolver is invoked whenever a connection is created, with the address that was
    requested (explicitly or by default). Embedding frameworks can use it to apply their
    own address configuration. Pass None to remove the hook.

    Args:
        resolver (callable): A function taking the requested address and returning the address to use.
...
def set_logger(new_logger: logging.Logger):
    ...
    Args:
        new_logger (logging.Logger): The logger to route all library logs through.

    Examples:
        set_logger(logging.getLogger("my_app.bridge"))
    """
    global logger
    logger = new_logger
...
class _ClientServer:
    ...
    def __init__(self, address: str = "unix:///var/run/arduino-router.sock"):
        ...
        address_config = _address_resolver(address) if _address_resolver else address
